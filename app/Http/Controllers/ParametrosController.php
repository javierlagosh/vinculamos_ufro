...
class ParametrosController extends Controller
{
    ...
    public function eliminarEscuelas(Request $request)
    {
        ...
        if (!$verificarDrop) {
            return redirect()->route('admin.listar.escuelas')->with('error', 'El departamento no se encuentra registrada en el sistema.');
        }

        $predrop = Carreras::where('escu_codigo', $request->escu_codigo)->first();
        if ($predrop) {
            return redirect()->route('admin.listar.escuelas')->with('error', 'El departamento está siendo ocupada en una carrera.');
        }

        $pre_drop = ParticipantesInternos::where('escu_codigo', $request->escu_codigo)->first();
        if ($pre_drop) {
            return redirect()->route('admin.listar.escuelas')->with('error', 'El departamento está siendo ocupada en una iniciativa.');
        }
        ...
        if (!$Drop) {
            return redirect()->back()->with('error', 'El departamento no se pudo eliminar, intente más tarde.');
        }

        return redirect()->route('admin.listar.escuelas')->with('exito', 'El departamento fue eliminada correctamente.');
    }
    ...
    public function actualizarEscuelas(Request $request, $escu_codigo)
    {
        ...
        // Verificar si la carrera existe
        if (!$escuela) {
            return redirect()->back()->with('error', 'El departamento no se encuentra registrada en el sistema.');
        }
        ...
        $validacion = $request->validate([
            ...
            'escu_nombre.required' => 'El nombre de El departamento es requerido.',
            'escu_nombre.max' => 'El nombre de El departamento excede el máximo de caracteres permitidos (255).',
        ...
        // Actualizar los campos de El departamento con los valores del formulario
        ...
        return redirect()->back()->with('exito', 'El departamento ha sido actualizada correctamente.');
    }
...
#
        ...
        return redirect()->back()->with('exito', 'El departamento fué creada existosamente');
    }
